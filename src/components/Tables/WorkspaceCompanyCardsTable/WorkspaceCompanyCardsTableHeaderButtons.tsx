...
type CompanyCardsSorting = ActiveSorting<string>;

type WorkspaceCompanyCardsSortByPopupProps = {
    columns: Array<{key: string; label: string; sortable: boolean}>;
    pendingSorting: CompanyCardsSorting;
    setPendingSorting: React.Dispatch<React.SetStateAction<CompanyCardsSorting>>;
...
const DEFAULT_COMPANY_CARDS_SORTING: CompanyCardsSorting = {
    columnKey: 'member',
...
function WorkspaceCompanyCardsFilterButton() {
    ...
    const selectedOption = statusFilterConfig.options.find((option) => option.value === currentFilter) ?? statusFilterConfig.options.find((option) => option.value === statusFilterConfig.default);
...
function WorkspaceCompanyCardsSortByPopup({
    columns,
    pendingSorting,
    setPendingSorting,
    onSortOrderPress,
    ...
    onApply,
    onReset,
}: WorkspaceCompanyCardsSortByPopupProps) {
    const styles = useThemeStyles();
    const {translate} = useLocalize();
...
function WorkspaceCompanyCardsDisplayButton() {
    ...
    const [pendingSorting, setPendingSorting] = useState<CompanyCardsSorting>(DEFAULT_COMPANY_CARDS_SORTING);
    ...
    const resetSorting = (closeOverlay: () => void) => {
    ...
    const applySorting = (closeOverlay: () => void) => {
...
function WorkspaceCompanyCardsTableHeaderButtons({policyID, feedName, isLoading, showTableControls, canWriteCompanyCards, CardFeedIcon, bulkActionsButton}: WorkspaceCompanyCardsTableHeaderButtonsProps) {
    const styles = useThemeStyles();
        ...
        <View>
            <View
                style={[
                    ...
                    styles.gap5,
                    styles.pb2,
                    !shouldShowNarrowLayout && [styles.pv2, styles.flexRow, styles.alignItemsCenter, styles.justifyContentBetween],
                ]}
            >
                {isLoading ? (
