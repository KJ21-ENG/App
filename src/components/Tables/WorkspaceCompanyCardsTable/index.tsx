...
function WorkspaceCompanyCardsTable({
    ...
    useEffect(() => {
        setSelectedCardKeys((previousSelectedCardKeys) => {
            if (previousSelectedCardKeys.length === 0) {
                return previousSelectedCardKeys;
            }

            const validCardKeys = new Set(cardsData.map((card) => card.keyForList));
            const nextSelectedCardKeys = previousSelectedCardKeys.filter((key) => validCardKeys.has(key));
            return nextSelectedCardKeys.length === previousSelectedCardKeys.length ? previousSelectedCardKeys : nextSelectedCardKeys;
        });
    }, [cardsData]);
    ...
    const confirmBulkUnassign = async () => {
        ...
        selectedAssignedCards.forEach((card) => {
